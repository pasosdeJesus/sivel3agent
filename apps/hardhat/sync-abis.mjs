...
function walkDir(dir) {
  ...
  list.forEach(file => {
    ...
    if (stat && stat.isDirectory()) {
      ...
      // Ensure we only grab the main contract JSON, not the debug files
...
// Main script
console.log("🔄 Syncing ABIs to the frontend app...")

try {
  // Find all artifact JSON files from Hardhat compilation
  ...
  artifactFiles.forEach(filepath => {
    ...
    // Extract just the ABI and save it to the Next.js directory
